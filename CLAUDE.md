...
- Conditional GET: anonymous content pages and feeds carry `ETag`/`Last-Modified` validators derived from the most recently updated post **and the last config edit** (`Response\latest_content_timestamp()` takes the `max()` of the latest post `updated` and `Config\config_modified_timestamp()`), and `Response\send_304_if_current()` short-circuits with `304 Not Modified`. The login page and 404 responses are excluded. Feeds use a longer `max-age` (`Response\feed_cache()`). `Config\save_ini_text()` stamps `updated` on the config row so settings changes invalidate cached pages immediately
