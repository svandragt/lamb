...
- **Listings.** Add `post-type: listing` to a post's front-matter, with an optional `price`, `currency`, `condition` and `contact`, to offer something for sale. The post page publishes schema.org `Product` and `Offer` markup so search engines and listings aggregators read it as commerce, and the listings appear on their own at `/listings`, `/listings/feed` and `/listings/feed.json` as well as in the main feed. Publishing a listing notifies your WebSub hub about the listings feed too, so subscribers are pushed new listings rather than waiting for a poll. A Micropub client creates one by posting an `h-product`.
