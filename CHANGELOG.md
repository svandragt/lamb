...
- **Listings.** Add `post-type: listing` to a post's front-matter, with an optional `price`, `currency`, `condition` and `contact`, to offer something for sale. The post page publishes schema.org `Product` and `Offer` markup so search engines and listings aggregators read it as commerce, and the listings appear on their own at `/listings`, `/listings/feed` and `/listings/feed.json` as well as in the main feed. Feed entries carry the price, condition and contact (as microformats markup, and as a `_listing` object in the JSON Feed), so an aggregator reads the terms from the feed itself. Publishing a listing notifies your WebSub hub about the listings feed too, so subscribers are pushed new listings rather than waiting for a poll. The price, condition and contact are shown together above the post body, so a listing that opens with a photo still states its price before the fold. Open your own listing while logged in and Lamb reports what its data actually says — a mistyped currency or condition is otherwise dropped in silence — and visitors never see the note. A price without a currency is published as an item without an offer rather than as an amount of unspecified money. A `contact` that is a URL renders as a link, so you can point buyers at a contact page instead of publishing an address — the value travels in both feeds as well as on the page. A Micropub client creates one by posting an `h-product`.
