...
`ROOT_URL` is in the key as a security measure, not to support serving several hosts — Lamb has never been built for that. An install with no `site_url` derives ROOT_URL from the request's Host header, which index.php already flags as attacker-chosen, and every `<loc>` is built from it. Uncached, a forged Host only spoils the response that sent it. Cached, it would be stored under the current content key and served to every later visitor until the content changed: a one-request cache poisoning, confirmed by removing the term and reproducing it. A configured `site_url` pins ROOT_URL and makes the term inert.
